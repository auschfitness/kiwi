...
describe('Settings — additional behaviour (still unconfigured)', () => {
  it('back button calls onBack', async () => {
    const onBack = vi.fn()
    render(<Settings onBack={onBack} onRetakePlacement={vi.fn()} />)
  ...
  it('calls onRetakePlacement when that button is pressed', async () => {
    ...
    render(<Settings onBack={vi.fn()} onRetakePlacement={onRetake} />)
  ...
  it('new-cards stepper disables decrease at 1 and increase at 20', () => {
    ...
    const { unmount } = render(<Settings onBack={vi.fn()} onRetakePlacement={vi.fn()} />)
